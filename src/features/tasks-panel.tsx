...
export function TasksPanel({ workspace, tasks, change, busy, edit, remove, categories }: { workspace: Workspace; tasks: Task[]; change: Change; busy: boolean; edit(task?: Task): void; remove(task: Task): void; categories(): void }) {
  ...
  return <Card className="panel tasks-panel"><Collapsible open={open} onOpenChange={setOpen}>
    <div className="section-heading">
      <h2><CollapsibleTrigger asChild><Button variant="ghost" className="heading-toggle">{open ? <ChevronDown /> : <ChevronRight />}Tasks <Badge variant="secondary">{tasks.length}</Badge></Button></CollapsibleTrigger></h2>
    ...
    <CollapsibleContent id="task-list">
    ...
    {!open && <p className="collapsed-summary">{tasks.filter(t => !t.completed).length} active tasks</p>}
