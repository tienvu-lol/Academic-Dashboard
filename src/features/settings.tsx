...
import { ArrowDown, ArrowUp, SlidersHorizontal, Sparkles } from 'lucide-react';
...
export function Settings({ workspace, change }: { workspace: Workspace; change: Change }) {
  ...
  const [typesafeKey, setTypesafeKey] = useState(settings.typesafeKey || '');
  ...
  return <Card className="panel settings-panel">
    <div className="section-heading"><div><div className="eyebrow">YOUR WORKFLOW, YOUR RULES</div><h2><SlidersHorizontal size={19} />Calendar priorities</h2></div></div>
    <p className="muted">Rules are applied from top to bottom. The first-ranked task in each day gets a colored outline.</p>
    ...
    <div className="section-heading" style={{ marginTop: '2rem' }}><div><div className="eyebrow">AI INTEGRATION</div><h2><Sparkles size={19} />TypeSafe Configuration</h2></div></div>
    <label className="field"><span>TypeSafe API Key</span><Input type="password" value={typesafeKey} onChange={e => { setTypesafeKey(e.target.value); setSaved(false); }} placeholder="sk-..." /><small className="muted">Locally stored key for Jev AI task prioritization.</small></label>

    <div className="form-actions"><span role="status" className="text-green">{saved ? 'Preferences saved' : ''}</span><Button disabled={busy} onClick={async () => { setBusy(true); const ok = await change(draft => { draft.dashboardSettings = { ...settings, sidebarSlim: settingsFor(draft).sidebarSlim, keywords: keywords.split(',').map(x => x.trim()).filter(Boolean), courseOrder, typesafeKey: typesafeKey.trim() || undefined }; }); setBusy(false); setSaved(ok); }}>Save preferences</Button></div>
  </Card>;
}
