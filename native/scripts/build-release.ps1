...
try {
    if (!(Test-Path 'node_modules/react-native-windows/package.json')) { throw 'Run npm ci inside native first.' }
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Windows compilation failed. See native/build-release.log.' }
