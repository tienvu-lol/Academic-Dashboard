...
try {
  ...
  await click('month');
  await waitFor("document.querySelectorAll('.full-calendar .planner-day').length === 42", 'shadcn month grid');
  ...
  await evaluate("document.querySelector('button[aria-label=\"Move Your calendar\"]').dispatchEvent(new DragEvent('dragstart', {bubbles:true,dataTransfer:new DataTransfer()}))");
  ...
  await evaluate("document.querySelector('button[aria-label=\"Resize Your tasks\"]').dispatchEvent(new KeyboardEvent('keydown', {bubbles:true,key:'ArrowDown'}))");
  await waitFor("document.querySelector('[data-widget=tasks] .widget-body')?.style.height === '340px'", 'keyboard widget resizing');
  await evaluate("document.querySelector('button[aria-label=\"Resize Your tasks\"]').scrollIntoView({block:'center'})");
  const grip = await evaluate("(() => { const r=document.querySelector('button[aria-label=\"Resize Your tasks\"]').getBoundingClientRect(); return {x:r.x+r.width/2,y:r.y+r.height/2}; })()");
  ...
  await send('Input.dispatchMouseEvent', { type: 'mouseMoved', x: grip.x, y: grip.y + 40, button: 'left', buttons: 1 });
  await waitFor("document.querySelector('[data-widget=tasks] .widget-body')?.style.height === '380px'", 'pointer resize preview');
  await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: grip.x, y: grip.y + 40, button: 'left', clickCount: 1 });
  await waitFor("!document.querySelector('button[aria-label=\"Resize Your tasks\"]')?.disabled", 'pointer resize saved');
  ...
  await click('Finish editing layout');
  await reloadPage();
  await waitFor("document.querySelector('[data-widget=calendar]')?.style.getPropertyValue('--widget-span') === '6' && document.querySelector('[data-widget=tasks] .widget-body')?.style.height === '380px'", 'layout sizes persisted after reload');
  ...
  await Bun.write(path.join(root, 'artifacts', production ? 'ui-production.png' : 'ui-development.png'), Buffer.from(screenshot.data, 'base64'));
  ...
  console.log('PASS: Electron render/isolation; task/date/completion; drag and keyboard ordering; green completed calendar/continuous headings; charts/heatmaps; widget docking, pointer/keyboard resize, independent layout persistence, narrow-window overflow; notes/credits; rejected application count/filter/chart.');
} finally {
  socket?.close();
